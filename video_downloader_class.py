...
class VideoDownloader:
    ...
    def download_video(self, video_url, audio_only, high_quality):
        ...
        if audio_only:
            ...
            print(f"{YouTube(video_url).title} downloaded: audio only")
        else:
            if high_quality:
                n = 0
                ...
                threads = list()
                threads.append(Thread(target=self.audio_only_download, args=(video_url, self.current_folder, True)))
                threads.append(Thread(target=self.video_only_download, args=(video_url,)))
                ...
                print(f"{YouTube(video_url).title} downloaded: progressive")
    ...
    def auto_download(self, video_url, audio_set, high_quality_set):
        ...
        print(f"Completed, all videos downloaded")
